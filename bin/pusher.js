...
(function() {
  var BUSY_REPORT_INTERVAL_TIME, CLIENT_TIMEOUT, DEBUG, EVENTS_TUBE, EventEmitter, JOB_LOG_FILENAME, KEEN_API_KEY, KEEN_PROJECT_ID, MAX_RETRIES, MAX_SHUTDOWN_DELAY, MIXPANEL_PROJECT_TOKEN, SILLY, SLACK_CHANNEL, SLACK_ICON_URL, SLACK_USERNAME, SLACK_WEB_API_TOKEN, VERSION, WORKER_COUNT, Worker, beanstalkHost, beanstalkPort, busyIntervalRef, createWorkers, figlet, getNumberOfWorkersBusy, getNumberOfWorkersRunning, gracefulShutdown, handleClosedWorker, logger, restartWorker, runAllWorkers, runBusyReport, running, startWorker, startWorkers, stopAllWorkers, translateLevel, winston, workerEvents, workers;

  beanstalkHost = process.env.BEANSTALK_HOST || '127.0.0.1';

  beanstalkPort = process.env.BEANSTALK_PORT || 11300;

  KEEN_PROJECT_ID = process.env.KEEN_PROJECT_ID || false;

  KEEN_API_KEY = process.env.KEEN_API_KEY || false;

  MIXPANEL_PROJECT_TOKEN = process.env.MIXPANEL_PROJECT_TOKEN || false;

  SLACK_WEB_API_TOKEN = process.env.SLACK_WEB_API_TOKEN || false;

  SLACK_USERNAME = process.env.SLACK_USERNAME || 'Swapbot';
  ...
  WORKER_COUNT = process.env.WORKER_COUNT || 5;

  DEBUG = !!(process.env.DEBUG || false);

  SILLY = !!(process.env.SILLY || false);

  EVENTS_TUBE = process.env.EVENTS_TUBE || 'tkevents';
  ...
  createWorkers = function() {
    ...
    results = [];
    for (i = j = 0, ref = WORKER_COUNT; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
  ...
  runBusyReport = function() {
    ...
    logger.info("Using " + numberOfWorkersRunning + " of " + WORKER_COUNT + " workers", {
      name: "usageReport",
  ...
  figlet.text("Tokenly Event Pusher", 'Slant', function(err, data) {
